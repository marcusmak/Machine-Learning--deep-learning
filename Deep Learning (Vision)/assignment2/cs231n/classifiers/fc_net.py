...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        num_train = X.shape[0]
        # minibatch = np.random.choice(X,num_batch) 
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
          ...
          #   print(f"{param} : {self.params[param].shape}")
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
          ...
          X , cache1 = affine_forward(X,W,b)
          if(self.use_batchnorm):
            gamma = self.params['gamma' + str(i+1)]
            beta = self.params['beta' + str(i+1)]
            X , cache2 = batchnorm_forward(X,gamma,beta,self.bn_params[i])
          else:
            cache2 = None
          X , cache3 = relu_forward(X) 
          if(self.use_dropout):
            X , cache4 = dropout_forward(X,self.dropout_param)
          else:
            cache4 = None
        ...
        for i in range(self.num_layers-1,0,-1):
          cache1 , cache2 , cache3 , cache4 = caches[i-1]
          str_W = 'W' + str(i)
          str_b = 'b' + str(i)
          W = self.params[str_W]
          loss += 0.5 * self.reg * (np.sum(W*W))
          ...
          if(self.use_dropout):
            dx = dropout_backward(dx,cache4)

          dx = relu_backward(dx, cache3)

          if(self.use_batchnorm):
            dx, dgamma , dbeta = batchnorm_backward_alt(dx,cache2)
            grads['gamma' + str(i)] = dgamma
            grads['beta' + str(i)]  = dbeta
            
          dx , dw , db = affine_backward(dx,cache1)

          grads[str_W] = dw + self.reg * W
          grads[str_b] = db
